import random

def popular_lista_aleatoria(lista, quantidade, faixa_inicial, faixa_final):
    """_summary_metodo que popula uma lista com numeros inteiros e aleatorios dentro de uma faixa

    Args:
        lista (int): armazena números inteiros e aleatórios 
        quantidade (int): total de numeros intieros a serem inseridos
        faixa_inicial (int): numero inicial de faixa
        faixa_final (int): numero final da faixa

    """

    for i in range(quantidade):
        lista.append( random.randint(faixa_inicial, faixa_final))

    pass

def exibir_lista(lista):
    """método que recebe uma lista e a exibe, elementos um abaixo do outro

    Args:
        lista (int): lista contando numeros inteiros
    """

    for item in lista:
        print(item)

    print("====================")
    print("Total de elementos: ", len(lista))

    

def copiar_lista_sem_replicados(lista_origem, lista_destino):
    """Método q copia o conteúdo da lista origem para dentro da lista destino retirando os itens relicados 

    Args:
        lista_origem (int): lista original contendo todos os némeros
        lista_destino (int): lista final contendo somente os némeros não replicados 
    """

    for item in lista_origem:
        if item not in lista_destino:
            lista_destino.append(item)
